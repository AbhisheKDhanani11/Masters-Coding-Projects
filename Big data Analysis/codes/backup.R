...
# getting car price and mileage grouped by the brand name
df_car_value <- df %>%
  group_by(Manufacturer.Name) %>%
  summarise(Price = Price, Mileage = Mileage.KM)
print(df_car_value)
ggplot(df_car_value, aes(x = df_car_value$Manufacturer.Name, y = df_car_value$Price,size = df_car_value$Mileage, colour = df_car_value$Manufacturer.Name)) +
  geom_jitter(width = 0.2, alpha = 0.9)
labs(title = "Scatter plot of price vs mileage", 
     x = "mileage", y = "price", color = df_car_value$Manufacturer.Name, size = df_car_value$Mileage) +
  theme_minimal()
